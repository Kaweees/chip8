...
class CPU {
  ...
  private:
    ...
    uint16_t pc = PROGRAM_START; // Program Counter - stores address of next instruction
                                 // to execute
  ...
  public:
    ...
    uint8_t read(uint16_t address) const { return mapper->fetchByte(address); }

    // Writes a byte to the memory at the specified address
    void write(uint16_t address, uint8_t value) {
      if (mapper == nullptr) { throw std::runtime_error("Memory bus not linked to CPU"); }
    ...
    void cycle() {
      ...
      auto elapsedTime = std::chrono::duration_cast<std::chrono::nanoseconds>(currentTime - lastCycleTime);

      if (elapsedTime < FRAME_DURATION) { std::this_thread::sleep_for(FRAME_DURATION - elapsedTime); }
    ...
    void fetch() {
      ...
      std::cout << "Opcode: " << std::uppercase << std::setfill('0') << std::setw(4) << std::hex << opcode << std::endl;
    }
        ...
        case 0x8000:
          switch (n) {
            ...
            case 0xE: // 0x8XYE: Shifts Vx left by one. VF is set to the most
              ...
              pc += 2;
              break;
            default: std::cout << "Unknown opcode: " << opcode << std::endl; break;
          }
          break;
        ...
        case 0xD000: { // 0xDXYN: Draws a sprite at (VX, VY) with a height of N
          ...
          for (int row = 0; row < n; ++row) {
            ...
            for (int col = 0; col < 8; ++col) {
              ...
              uint8_t displayPixel = mapper->display.getPixel(pixelX + col, pixelY + row);
              if (spritePixel && displayPixel) { v[0xF] = 1; }
              mapper->display.setPixel(pixelX + col, pixelY + row, spritePixel ^ displayPixel);
            }
          }
          pc += 2;
          break;
        ...
        case 0xF000:
          switch (nn) {
            ...
            case 0x55: // 0xF055: Stores V0 to VX (including VX) in memory starting
                       // at address I
              for (uint8_t j = 0; j <= x; ++j) { write(i + j, v[j]); }
              pc += 2;
              break;
